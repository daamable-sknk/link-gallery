...
업데이트 시각은 `src/data/meta.json`에 저장되며, 갤러리 페이지 상단과 README 배지에 표시된다.

자동 갱신 스케줄 (맥북이 켜져 있을 때, **하루 1회**):

| 구분 | 실행 시각 |
|------|-----------|
| 평일 (월~금) | 오전 10:00 |
| 주말 (토·일) | 오후 3:00 |
| 예정 시각을 놓친 경우 | 맥을 켤 때 바로 실행 |

연휴가 평일에 끼어 있어도 10시에 컴퓨터를 안 켜면, 나중에 맥을 켜는 순간 업데이트가 돌아간다.
